...
module ibex_controller_eti_assertions (
  ...
  // -----------------------------------------------------------------------
  // NOTE: All 17 NS31A assertions from groups 1-17 were flagged as
  // UNTRANSLATABLE because the required internal signals (exception flags,
  // CSR write data, mstatus fields, mepc, interrupt counter, etc.) are not
  // exposed as available signals in the Ibex ibex_controller_eti module.
  // No assertions could be generated without inventing signal names or
  // making unsupported assumptions about internal implementation.
  // -----------------------------------------------------------------------

  // Placeholder assertion to maintain module validity (always passes)
  property eti_SEC_PLACEHOLDER;
    @(posedge clk_i) disable iff (!rst_ni)
    1'b1 |-> 1'b1;
  endproperty
  assert property (eti_SEC_PLACEHOLDER);
